...
- **自动触控层（新）**：扫描 `data.win` 字节码，自动识别游戏**真正使用的键盘按键**（不同游戏用的键不一样），只为这些键生成触控按钮；方向键 / WASD 自动生成摇杆。全部为自绘图形，不依赖任何精灵资源，兼容 GMS 1.4 ~ 2024.x
- **EDIT 编辑模式（新）**：游戏内右上角齿轮**长按 0.7 秒**进入编辑模式，可拖动按钮与摇杆、调整按钮 / 摇杆大小与透明度、单独显示或隐藏某个按钮、一键**关闭整个触控层**；布局自动保存到 `gmm_touch.ini`
- **移动端性能优化（新）**：关闭纹理插值与重复、降低圆形绘制精度、限制 `application_surface` 分辨率上限（≤1080p），让低端手机也能流畅运行，**无需 YYC 编译器**
